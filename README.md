...
|-----------|-------|--------|-------|
| Zeyrek candidate generation | pending SLURM run [^zeyrek-hw] | ~267 MB | Run `sbatch scripts/truba/submit_zeyrek_benchmark.sh` on Orfoz |
...
[^zeyrek-hw]: Prior login-node figure (1,209 tok/s) is a diagnostic value, not a README claim. Orfoz authoritative run pending.
...
| Zeyrek throughput | `scripts/benchmark_zeyrek.py` | `audit/benchmark_results/zeyrek_throughput.json` | ⏳ pending Orfoz run |
| Training wall-clock | `scripts/truba/submit_train_disambiguator.sh` | `models/v6_retimed/training_log.json` | ⏳ pending Orfoz run |
...
| Inference (Zeyrek candidate gen) | Any laptop (CPU) | pending Orfoz measurement |
